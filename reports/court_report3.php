<?php
	...
	*/
	...
	$r=$orm->cases()->where('user_id', $usr['id'])->where('command_id',$checked)->where('(auto_received_num is null) or (auto_received_num=?)','')->where('(date_received3 is null) or (date_received3=?)','0000-00-00');
	...
	$cases=$r->toArray();
    $html= $twig->render('court_report3.html', array('report_name'=>'รายงาน','date1'=>$date1,   'date2'=>$date2, 'type_id'=>$type_id, 'type_item'=>$atype,  'cases'=>$cases, 'user'=> $_SESSION['user']));
	printPdf($html);
	//echo $html;