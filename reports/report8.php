<?php
	...
	$checkedids=$orm->at()->select('id')->where('checked',1);
	$r=$orm->cases()->order('id asc')->where('command_id',$checkedids)->where('auto_received_num=?','')->where('((date_received3 is null) or (date_received3=?))','0000-00-00');
	...
	$cases=$r->toArray();
    $html=$twig->render('report8.html', array('report_name'=>'รายงาน','date1'=>$date1,  'date2'=>$date2, 'cases'=>$cases,'court'=>$court));
	printPdf($html);