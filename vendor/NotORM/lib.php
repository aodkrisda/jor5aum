...
class NotORM extends NotORM_Abstract {
  ...
  private $table_prefix=MYSQL_TABLE_PREFIX; 
  ...
  public function addPrimary($table, $pk){
    if($table){
      ...
      if($pk){
        self::$_PRIMARY[$table]=$pk;
      }else{
        unset(self::$_PRIMARY[$table]);
      }
    }
  }
  ...
  public static function getPrimary( $table) {
    if($table){
       $th=self::getInstance();
       $table=$th->get_short_name($table);
       if(isset(self::$_PRIMARY[$table])){
	...
	public static function getRefence($pk, $table) {
    if($table){
       $th=self::getInstance();
       $table=$th->get_short_name($table);
       $fpk=$table . '.' . $pk;
       if(isset(self::$_REFERENCE[$fpk])){
          return self::$_REFERENCE[$fpk];       
       }else if(isset(self::$_REFERENCE[$pk])){
          return self::$_REFERENCE[$pk];
       }
     }
		 return null;
	}
  ...
  public function get_table_name($table, $add_prefix=true){
    if($add_prefix && $this->table_prefix){
  ...
  public function get_short_name($table){
    if($this->table_prefix){
      if(strpos($table,$this->table_prefix)===0){
         $table=substr($table,strlen($this->table_prefix));
      }
    }
    return $table;
  }
