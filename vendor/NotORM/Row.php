...
class NotORM_Row extends NotORM_Abstract implements IteratorAggregate, ArrayAccess, Countable, JsonSerializable {
	...
	*/
	...
	function fetch(){
		if (array_key_exists($this->result->primary, $this->row)) {
			$result=$this->result->push()->where('')->where($this->result->primary, $this->primary);
			if(count($result->where($this->result->primary, $this->primary))>0){
				$row=$result->fetch();
				foreach($row as $k=>$v){
					$this->row[$k]=$v;
				}
				...
				foreach($keys as $key) unset($this->row[$key]);
				unset($keys);
			}
			unset($result);
		}
