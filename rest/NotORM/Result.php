...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable, JsonSerializable {
	...
	protected $unPrefixTable,$unPrefixTableQuote,$tableQuote;
	protected $internal_fields=null;
	...
	*/
	function __construct($table, NotORM $notORM, $single = false) {
		...
		$this->notORM = $notORM;
		$this->single = $single;
		$this->unPrefixTable=$this->stripPrefixTable($this->table);

		$this->unPrefixTableQuote=$this->__quoteTable($this->unPrefixTable);
		$this->tableQuote=$this->__quoteTable($table);
		$this->primary = $notORM->structure->getPrimary($this->unPrefixTable);
	}
	function __quoteTable($tb){
		$str=$tb;
		if(!preg_match('/\S{1,}\s+(left|right|inner|outer)\s+join\s+(\S{1,}(\s+as\s+\S+){0,1})\s+on\s+(\S{1,})\s{0,}=\s{0,}(\S{1,})/i', $str)){
			if(!preg_match('/(`|\.)/',$str)){
				$str="`{$str}`";
			}
		}	
	...
	*/
	...
	protected function stripPrefixTable($table){
		if($table){
			preg_match_all('/^(\S{1,})(\s+as\s+){0,}/i', $table, $ms);
			if($ms && isset($ms[1])){
				$table=implode($ms[1],'');
			}	
			$prefix=$this->notORM->structure->getTablePrefix();
			if($prefix){
	...
	protected function createJoins($val) {
		$return = array();
		//hack custom join
		if(preg_match('/\S{1,}\s+(left|right|inner|outer)\s+join\s+(\S{1,}(\s+as\s+\S+){0,1})\s+on\s+(\S{1,})\s{0,}=\s{0,}(\S{1,})/i', $this->table)){
			return $return;
		}
		...
		foreach ($matches[1] as $names) {
			...
			if ($names != "$parent.") { // case-sensitive
				...
				foreach ($matches as $match) {
					...
					$table=$this->__quoteTable($table);
					$name=$this->__quoteTable($name);
	...
	*/
	function __toString() {
		...
		$alias='';
		if($this->unPrefixTableQuote!=$this->tableQuote){
			if(! preg_match('/\S{1,}\s+(left|right|inner|outer)\s+join\s+(\S{1,}(\s+as\s+\S+){0,1})\s+on\s+(\S{1,})\s{0,}=\s{0,}(\S{1,})/i', $this->table)){
	...
	*/
	function insert_multi(array $rows) {
		...
		if($rows){
			$tb_fields=$this->get_fields();
			if($tb_fields && is_array($tb_fields)){
				foreach($rows as &$row){
					if(is_array($row)){
						$keys=array_keys($row);
						foreach($keys as $key){
	...
	*/
	...
	function toArray(){
		$rs=array();
		foreach($this as $r) $rs[]=$r->toArray();
		return $rs;
	}
	...
	function get_fields(){
		if($this->internal_fields===null){
			if(is_callable($this->notORM->get_fields)){
				$this->internal_fields=$this->notORM->get_fields($this->table);
			}
		}
